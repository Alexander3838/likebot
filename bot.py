...
4. Не жульничай 😉", parse_mode="HTML")
        ...
        for uid, link in queue:
            if uid == user_id or link in users[user_id]["likes_given"]:
                continue
            tasks.append(link)
            if len(tasks) == 3:
                break
...
⏳ Через 20 сек. нажми '✅ Подтвердить лайки'")
        ...
        now = time.time()
        if "task_time" not in users[user_id] or now - users[user_id]["task_time"] < 20:
        ...
        top = sorted(users.items(), key=lambda x: len(x[1]['likes_given']), reverse=True)
